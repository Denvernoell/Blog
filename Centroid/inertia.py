    ...
    def get_bounds(self):
        ...
        if self.inertia_type == 'I_y':
            ...
            if self.axis_type == 'axis':
                ...
                self.p_axis = (
                    self.shape.area * (self.shape.centroid.x - self.full_shape.centroid.x)**2)
            #
            ...
            if self.axis_type == 'axis':
                ...
                self.p_axis = (
                    self.shape.area * (self.shape.centroid.y - self.full_shape.centroid.y)**2)
...
class full_shape:
    def __init__(self, shapes, inertia_type, axis_type):
        ...
        for i, shape in enumerate(shapes):
            c_shape = single_shape(
                shape, i+1, self.shape, inertia_type, axis_type)
        ...
        for i, shape in enumerate(shapes):
            ...
            if shape['sign'] == '+':
                ...
                ax.text(S.centroid.x, S.centroid.y,
                        f"{i+1}", fontsize=10, bbox=dict(facecolor='green', alpha=1))
            else:
                ax.add_artist(PolygonPatch(S, facecolor='red', alpha=0.3))
                ax.text(S.centroid.x, S.centroid.y,
                        f"{i+1}", fontsize=10, bbox=dict(facecolor='red', alpha=.3))
        # self.my_ax = ax
        self.
        fig = fig
